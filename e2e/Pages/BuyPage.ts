...
class BuyPage {
  ...
  constructor(page: Page) {
    ...
    // LOCATORS
    this.commonAdaptiveSwitcherLocator = (labels: string[]) => this.page.locator('[data-test="adaptive-switcher__switcher"]')
    ...
    this.ideaProductCard = (productCardTitle: string) => this.page.locator('[data-test^="product-card-"]')
      .filter({ has: this.page.locator('[data-test^="product-name"]')
      .filter({ hasText: productCardTitle }) }).first(); // TODO: This could have a unique data-testid attribute in the future
    this.rustRoverProductCards = this.page.locator('div.wt-row.wt-row_align-items_stretch.wt-row_size_m > div'); // TODO: This could have a unique data-testid attribute in the future
    this.cLionNonCommercialProductCard = this.page.locator('[data-test="non-commercial-product-buy-card-CLion-Non-Commercial"]');
    this.cLionCommercialProductCard = this.page.locator('[data-test="non-commercial-product-buy-card-CLion-Commercial"]');
    this.cLionAllProductsPackProductCard = this.page.locator('[data-test="sticky-tag-wrapper"]')
      .filter({ has: this.page.locator('[data-test^="product-name"]')
      .filter({ hasText: 'All Products Pack' }) }).first(); // TODO: This could have a unique data-testid attribute in the future
    this.cLionNonCommercialProductCardDownloadButton = this.cLionNonCommercialProductCard.locator('[data-test="button"]');
  }
  ...
  async validateCommonTierSwitcher(productName: string) {
    switch (productName) {
      case "idea": {
        await this.validateCommonAdaptiveSwitcher(TIER_LABELS, 'idea-common-tier-switcher.aria.yml');
        break;
      }
      case "rustRover": {
        await this.validateCommonAdaptiveSwitcher(TIER_LABELS, 'rustrover-common-tier-switcher.aria.yml');
        break;
      }
      case "cLion": {
      ...
      default:
        throw new Error(`Invalid product name: ${productName}. Valid product names are "idea", "rustRover", or "cLion".`);
    }
  }
  ...
  async validateBillingTermSwitcher(productName: string) {
    switch (productName) {
      case "idea":
        await this.validateCommonAdaptiveSwitcher(BILLING_LABELS, 'idea-billing-term-switcher.aria.yml');
        break;
      case "rustRover":
        await this.validateBillingTypeRadioGroup('rustrover-billing-type-radio-group.aria.yml');
        break;
      case "cLion":
        await this.validateCommonAdaptiveSwitcher(BILLING_LABELS, 'clion-billing-term-switcher.aria.yml');
        break;
      default:
        throw new Error(`Invalid product name: ${productName}. Valid product names are "idea", "rustRover", or "cLion".`);
    }
  ...
  async validateElementDisplayed(locator: Locator, state: string) {
    if (state === DISPLAYED) {
      ...
      throw new Error(`Invalid state: ${state}. Valid states are "${DISPLAYED}" or "${NOT_DISPLAYED}".`);
    }
  }
  ...
  async validateIdeaProductCard(cardTitle: string, tierType: string, billingTerm: string) {
    if (!['commercial', 'personal'].includes(tierType)) {
      throw new Error(`Invalid tier type: ${tierType}. Valid tier types are "commercial" or "personal".`);
    }
    if (!['monthly', 'yearly'].includes(billingTerm)) {
      throw new Error(`Invalid billing term: ${billingTerm}. Valid billing terms are "monthly" or "yearly".`);
    }
  ...
  async validateCLionProductCard(cardTitle: string, tierType: string, billingTerm: string) {
    ...
    await this.validateClionProductCardTierSwitchers('clion-commercial-product-card-tier-switcher.aria.yml');
  }
  ...
  getProductShortCode(cardTitle: string): string {
    ...
    switch (cardTitle) {
      ...
      case CLION_COMMERCIAL:
        return 'CL';
      default:
        throw new Error(`Invalid card title: ${cardTitle}. Valid card titles are ${INTELLIJ_IDEA_ULTIMATE}, ${RUSTROVER_COMMERCIAL}, ${CLION_COMMERCIAL}, or ${ALL_PRODUCTS_PACK}.`);
    }
  }
  ...
  rustRoverProductCardLocatorMapper(cardTitle: string): Locator {
    switch (cardTitle) {
      ...
      default:
        throw new Error(`Invalid card title: ${cardTitle}. Valid card titles are "RustRover non-commercial use", "RustRover", or "All Products Pack".`);
    }
  }
  ...
  cLionProductCardLocatorMapper(cardTitle: string): Locator {
    switch (cardTitle) {
      ...
      default:
        throw new Error(`Invalid card title: ${cardTitle}. Valid card titles are "CLion Non-Commercial", "CLion Commercial", or "All Products Pack".`);
    }
  }

  async validateProductCardCommons(card: Locator, tierType: string, billingTerm: string, cardTitle: string) {
    if (cardTitle === INTELLIJ_IDEA_ULTIMATE || cardTitle === CLION_COMMERCIAL) {
      await expect(card.getByRole('checkbox', { name: 'Supercharge with JetBrains AI Pro' })).toBeVisible();
    }
    ...
    if(cardTitle === CLION_NON_COMMERCIAL) {
      await expect(this.cLionNonCommercialProductCardDownloadButton).toHaveAttribute('href', '/clion/download/');
    }

    // validate buy and get quote links
    if (cardTitle === RUSTROVER_COMMERCIAL || cardTitle === ALL_PRODUCTS_PACK || cardTitle === INTELLIJ_IDEA_ULTIMATE || cardTitle === CLION_COMMERCIAL) {
      const productCode = this.getProductShortCode(cardTitle);
      const itemCode = this.generateItemCode(productCode, tierType as TierType, billingTerm as BillingTerm);
  ...
  async validateClionProductCardTierSwitchers(ariaFileName: string) {
    const snapshot = await readAriaSnapshot(ariaFileName);
    await expect(this.cLionCommercialProductCard.locator('[data-test="adaptive-switcher__switcher"]')).toMatchAriaSnapshot(snapshot);
    await expect(this.cLionAllProductsPackProductCard.locator('[data-test="adaptive-switcher__switcher"]')).toMatchAriaSnapshot(snapshot);
  }

  async validateProductCardBuyLink(card: Locator, itemCode: string) {
    await expect(card.getByRole('link', { name: 'Buy' })).toHaveAttribute('href', new RegExp(`shop\\/buy\\?item=${itemCode}`));
  }

  async validateProductCardGetQuoteLink(card: Locator, itemCode: string) {
    await expect(card.getByRole('link', { name: 'Get quote' })).toHaveAttribute('href', new RegExp(`shop\\/quote\\?item=${itemCode}`));
  }
